"""
Workplace EQ lesson page (shared English+ markup since the 2026 redesign;
the look lives in shared/theme.css + the chrome block in course-engine.js).
...
NUM_WORDS = {3: "three", 4: "four", 5: "five", 6: "six", 7: "seven", 8: "eight", 9: "nine", 10: "ten"}
...
def drop_elsewhere(html):
    """The rules' "Elsewhere" tip now lives in the culture notes card."""
    return re.sub(r'<div class="tip-box"[^>]*>\s*🌍.*?</span></div>', "", html, flags=re.S)
...
def render_lesson(d):
    ...
    <div class="card">
      ...
      <p style="font-size:13.5px;color:var(--text-secondary);margin-bottom:14px">{c.get("instruction") or "Hover over (or tap) each line to see how a British listener is likely to hear it."}</p>
      <div id="compare-card" style="font-size:14.5px;line-height:1.9"></div>
    </div>'''
    culture = CULTURE.get(d["num"])
    if culture:
        cols = "".join(f'<div><div class="culture-h">{h}</div><p>{t}</p></div>' for h, t in culture)
        compare_html += f'''
    <div class="card culture-card">
      <h3>Across cultures</h3>
      <div class="culture">{cols}</div>
    </div>'''
    ...
    <div class="card">
      ...
      <div class="expand-content" id="rules-content">
        <div style="margin-top:16px">
          {drop_elsewhere(widget["rules_html"])}
        </div>
      </div>
    </div>
    {compare_html}
  ...
  <section class="section" id="sec-vocab">
    ...
    <div class="card">
      <p style="font-size:14.5px;margin-bottom:16px">All {NUM_WORDS[len(d["reading"]["vocab_data"])]} words from the text — mixed formats.</p>
  ...
  <section class="section" id="sec-practice">
    ...
    <div class="card">
      ...
      <p style="font-size:12.5px;text-transform:uppercase;letter-spacing:.03em;color:var(--text-tertiary);margin-bottom:16px">Focus — {practice["gapfill_focus"]}</p>
      {practice_gap_html}
    </div>
    {second_html}
    <div class="card">
  ...
  <section class="section" id="sec-wrapup">
    ...
    body = "\n".join([
        ...
        f'\n  <section class="section" id="sec-diagnostic">\n    <div class="section-label"><span class="num">2</span> Quick check — before we explain anything</div>\n    <div class="card">\n      <p style="margin-bottom:16px;font-size:14.5px;color:var(--text-secondary)">{NUM_WORDS[len(d["diagnostic"])].capitalize()} quick questions. This isn\'t graded harshly — it just shows us where to focus.</p>\n      {diag_qs}\n    </div>\n  </section>',
        concept_html,
        reading_html,
        vocab_html,
        practice_html,
  ...
  Course.renderCategorise('cat-root', {j(second_widget["items"])}, {j(second_widget["categories"])}, {{lessonId: LESSON_ID, sectionPrefix: 'practice-cat'}});'''
    ...
    words: {j(item["words"])}, errorIndices: {j(item["error_indices"])}, correction: {j(item.get("correction", ""))}{(", explanation: " + j(item["explanation"])) if item.get("explanation") else ""},
    restore: sectionMap['practice-err-{i+1}'] ? {{answerGiven: sectionMap['practice-err-{i+1}'].answer_given}} : null
  }});'''
            )
        second_js = "\n".join(parts)
